...
testable without mocks. The native C plugin (`plugin/fractisynth/`) and the obspython
script (`plugin/synthobs/`) **mirror** this math — they never re-implement it.
...
| `provenance.py` | Telemetry-record packing, SHA-256 checksum, LSB/visible-signature embedding, tamper evidence. |

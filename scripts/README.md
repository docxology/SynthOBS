...
| `promote_obs_evidence.py` | Fail-closed promotion boundary from a passing live scenario directory into the versioned evidence bundle `docs/manuscript/assets/obs/`; recomputes hashes, PNG dimensions, and WAV metadata | self-contained promotion gate (`synthobs.live_scenario.v2` manifest) | `uv run python scripts/promote_obs_evidence.py output/live/<ts>` |
...
reads `docs/scholarship_sources.json` + `docs/manuscript/references.bib` and delegates.
